  ...
  for(int i=0;i<27;i++){
    for(int j=0;j<20;j++){
      ...
      if(pos==1){
        // for(int m=0;m<27;m++){
        //   for(int n=0;n<20;n++){      
        //     cout<<digits[m][n]<<endl;
        //   }
        // }
        // cout<<"matchOne "<<i<<' '<<pos<<' '<<j<<' '<<digits[i][j]<<' '<<input[pos+j]<<' '<<matchOne[i][pos][j]<<endl;
      }
    }
  }
  return;
...
int main(){
  ...
  for(int s=N-19+1;s>0;s--){
    ...
    if(minC>MAXC){
      ...
      cout<<s<<' '<<tag<<' '<<state[s]<<' '<<matchS[s]<<endl;
    }
  }
  fout<<matchS[1]<<endl;
  return 0;
